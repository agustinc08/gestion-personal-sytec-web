...
export default function AdminV2Panel() {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap gap-2">{[
    ['summary', 'Resumen ejecutivo', Gauge], ['audit', 'AuditorÃ­a', ShieldCheck], ['exports', 'Exportaciones', Download], ['settings', 'ConfiguraciÃ³n', Settings],
...
function ExportsView() {
  ...
  const resources = [['employees','Empleados'],['licenses','Licencias'],['work-logs','Parte diario'],['projects','Proyectos'],['dependencies','Dependencias'],['technology-stats','TecnologÃ­as / stacks'],['strike-duty','Guardias / paro'],['audit-logs','AuditorÃ­a']];
  const download = async (resource: string) => { setLoading(resource); setMessage(''); try { await downloadExport(resource, { from, to }); setMessage('ExportaciÃ³n generada correctamente.'); } catch { setMessage('No se pudo generar la exportaciÃ³n. IntentÃ¡ nuevamente.'); } finally { setLoading(''); } };
  return <section className="rounded-2xl border bg-white p-5"><h2 className="text-lg font-black">Exportaciones Excel</h2><p className="text-xs text-slate-500">Los reportes excluyen contraseÃ±as, hashes y tokens.</p><div className="mt-4 flex flex-wrap gap-2"><label className="text-xs">Desde<input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="ml-2 rounded-lg border p-2" /></label><label className="text-xs">Hasta<input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="ml-2 rounded-lg border p-2" /></label></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{resources.map(([resource,label]) => <button key={resource} onClick={() => download(resource)} disabled={!!loading} className="flex items-center justify-center gap-2 rounded-xl border bg-slate-50 p-4 text-xs font-bold hover:border-indigo-300"><FileText className="h-4 w-4 text-emerald-600" />{loading === resource ? 'Generando...' : `Exportar ${label}`}</button>)}</div>{message && <p className="mt-4 rounded-xl bg-slate-50 p-3 text-xs">{message}</p>}<p className="mt-4 text-[10px] text-slate-400">PDF queda pendiente; Excel es el formato principal de esta versiÃ³n.</p></section>;
}
...
function SettingsView() {
  ...
  const save = async (row: any, raw: string | boolean) => { let value: any = raw; try { if (row.type === 'number') value = Number(raw); if (row.type === 'json') value = JSON.parse(String(raw)); await settingsApi.update(row.key, value); setRows(await settingsApi.all()); setMessage('Cambios guardados correctamente.'); } catch { setMessage('No se pudo guardar. IntentÃ¡ nuevamente.'); } };
  if (loading) return <Empty text="Cargando configuraciÃ³n..." />;
  return <section className="rounded-2xl border bg-white p-5"><h2 className="text-lg font-black">ConfiguraciÃ³n del sistema</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{rows.map((row) => <label key={row.key} className="rounded-xl border p-4 text-xs font-bold">{row.label}<span className="block text-[10px] font-normal text-slate-400">{row.description || row.key}</span>{row.type === 'boolean' ? <input type="checkbox" checked={Boolean(row.value)} onChange={(e) => save(row, e.target.checked)} className="mt-3" /> : <input defaultValue={row.type === 'json' ? JSON.stringify(row.value) : String(row.value)} onBlur={(e) => save(row, e.target.value)} className="mt-3 w-full rounded-lg border px-3 py-2 font-normal" />}</label>)}</div>{message && <p className="mt-4 rounded-xl bg-slate-50 p-3 text-xs">{message}</p>}</section>;
}
